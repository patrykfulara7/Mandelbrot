...
namespace Automata {
    Window::Window(int width, int height, const std::string &title) : data(width, height, title) {
        ...
        data.windowHandle = glfwCreateWindow(width, height, title.c_str(), glfwGetPrimaryMonitor(), nullptr);
        ...
        glfwSetMouseButtonCallback(data.windowHandle, [](GLFWwindow *window, int button, int action, int mods) {
            auto data = static_cast<WindowData *>(glfwGetWindowUserPointer(window));
        ...
        glfwSetCursorPosCallback(data.windowHandle, [](GLFWwindow *window, double x, double y) {
            auto data = static_cast<WindowData *>(glfwGetWindowUserPointer(window));
